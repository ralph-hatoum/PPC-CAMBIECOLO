...
class offre:
    ...
    def __str__(self):
        if self.avaiable:
            return "\033[36mOFFER_ID = " + str(self.id_offer) + "\033[0m for \033[91m\033[1m" \
...
def player(id_player, offers_list, offers_lock_dict, last_offer_id, nbr_players):
    ...
    while playing:
        ...
        if interaction == "make_offer":
            make_offer(id_player, offers_list, offers_lock_dict, last_offer_id, nbr_players)

        if interaction == "accept_offer":
            accept_offer(id_player, offers_list, offers_lock_dict)

        if interaction == "display_offers":
            display_offers(id_player + 2, mq, offers_list)
...
def display_offers(t, message_queue, offers_list):
    mes = "\n".join([str(offers_list[i]) for i in range(len(offers_list))])
    mes = mes.encode()
...
def make_offer(id_player, offers_list, offers_lock_dict, last_offer_id, nbr_players):
    ...
    id_offer = last_offer_id.value
    last_offer_id.value += 1
    ...
    for i in range(nbr_players):
        display_offers(i + 2, receiver, offers_list)

    offer_lock.release()


def accept_offer(id_player, offers_list, offers_lock_dict):
    ...
    if conclusion == "NO_DEAL":
        ...
        offer_lock.acquire()
        offers_list.[offer_id].avaiable = False
